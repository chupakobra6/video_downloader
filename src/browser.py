...
class BrowserProfileManager:
    ...
    def _has_cookies(self, dir_name: str) -> bool:
        ...
        if not profile_dir.exists():
            logger.debug(
                ...
                extra={"profile": dir_name, "path": str(profile_dir)},
            )
            return False
    ...
    def _map_display_name_to_profile_dir(self, display_name: str) -> Optional[str]:
        ...
        try:
            ...
            info_cache: dict[str, Any] = (
                (data or {}).get("profile", {}).get("info_cache", {})
            )
            target: str = display_name.strip().casefold()
            ...
            if info_cache:
                candidates = [
                    f"{dir_name} -> name='{meta.get('name', '')}', "
                    f"gaia='{meta.get('gaia_name', '')}'"
                    for dir_name, meta in info_cache.items()
                    if isinstance(meta, dict)
                ]
                logger.info(
                    "Available browser profiles",
                    extra={"candidates": "; ".join(candidates)},
                )

        except Exception as e:
            logger.exception(
                "Failed to map profile name",
                extra={"display_name": display_name, "error": str(e)},
            )
    ...
    def _get_display_name_for_dir(self, dir_name: str) -> Optional[str]:
        ...
        try:
            ...
            meta: Any = info_cache.get(dir_name)
            ...
            logger.exception(
                ...
                extra={"dir_name": dir_name, "error": str(e)},
            )
    ...
    def find_profile_name(self, requested_profile: Optional[str]) -> Optional[str]:
        ...
        if not self._base_path:
            logger.info(
                "No base path found, returning requested profile",
                extra={"profile": requested_profile},
            )
        ...
        if requested_profile:
            if self._has_cookies(requested_profile):
                logger.info(
                    "Found direct profile match", extra={"profile": requested_profile}
                )
                return requested_profile
            ...
            if mapped and self._has_cookies(mapped):
                logger.info(
                    "Found mapped profile",
                    extra={"requested": requested_profile, "mapped": mapped},
                )
                return mapped
    ...
    def get_profile_info(self, profile_name: Optional[str]) -> dict[str, Optional[str]]:
        ...
        display_name: Optional[str] = self._get_display_name_for_dir(profile_name)
        return {
            "name": profile_name,
