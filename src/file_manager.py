...
class FileManager:
        ...
        for partial_path in self._get_partial_paths(final_path):
            try:
                if partial_path.exists():
                    ...
                    logger.debug(
                        "Removed partial file", extra={"path": str(partial_path)}
                    )
            except Exception as e:
                logger.warning(
                    "Failed to remove partial file",
                    extra={"path": str(partial_path), "error": str(e)},
                )
    ...
    def _cleanup_artifacts(self, final_path: Path) -> None:
        ...
        try:
            ...
            logger.exception(
                ...
                extra={"final_path": str(final_path), "error": str(e)},
            )
                ...
                if name.endswith(".ytdl"):
                    ...
                    if (directory / base).exists():
                        try:
                            child.unlink(missing_ok=True)
                            logger.debug(
                                "Swept sidecar file", extra={"path": str(child)}
                            )
                        except Exception as e:
                            logger.warning(
                                "Failed to sweep sidecar file",
                                extra={"path": str(child), "error": str(e)},
                            )
                    continue
                ...
                if ".part" in name:
                    ...
                    if (directory / base).exists():
                        try:
                            child.unlink(missing_ok=True)
                            logger.debug(
                                "Swept partial file", extra={"path": str(child)}
                            )
                        except Exception as e:
                            logger.warning(
                                "Failed to sweep partial file",
                                extra={"path": str(child), "error": str(e)},
                            )

        except Exception as e:
            logger.exception(
                "Leftovers sweep failed",
    ...
    def should_skip_download(self, expected_path: Path) -> bool:
        ...
        if has_partials:
            logger.info(
                "Found partial files, will resume download",
